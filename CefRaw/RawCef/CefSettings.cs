...
/// <summary>
/// Represents a client-owned instance of <c>_cef_settings_t</c>.
/// </summary>
public unsafe partial class CefSettings : ICefSettings, IDisposable
{
    private _cef_settings_t* _ptr;
    private int _disposed;

    public CefSettings()
    {
        _ptr = (_cef_settings_t*)NativeMemory.AllocZeroed((nuint)sizeof(_cef_settings_t));
        _ptr->size = (nuint)sizeof(_cef_settings_t);
        Initialize();
    }
    ...
    public void Dispose()
    {
        ...
        if (_ptr is not null)
        {
            CefUnsafe.StringUtf16Clear(&_ptr->browser_subprocess_path);
            CefUnsafe.StringUtf16Clear(&_ptr->framework_dir_path);
            CefUnsafe.StringUtf16Clear(&_ptr->main_bundle_path);
            CefUnsafe.StringUtf16Clear(&_ptr->cache_path);
            CefUnsafe.StringUtf16Clear(&_ptr->root_cache_path);
            CefUnsafe.StringUtf16Clear(&_ptr->user_agent);
            CefUnsafe.StringUtf16Clear(&_ptr->user_agent_product);
            CefUnsafe.StringUtf16Clear(&_ptr->locale);
            CefUnsafe.StringUtf16Clear(&_ptr->log_file);
            CefUnsafe.StringUtf16Clear(&_ptr->javascript_flags);
            CefUnsafe.StringUtf16Clear(&_ptr->resources_dir_path);
            CefUnsafe.StringUtf16Clear(&_ptr->locales_dir_path);
            CefUnsafe.StringUtf16Clear(&_ptr->accept_language_list);
            CefUnsafe.StringUtf16Clear(&_ptr->cookieable_schemes_list);
            CefUnsafe.StringUtf16Clear(&_ptr->chrome_policy_id);
