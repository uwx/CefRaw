...
internal static class TypeMapper
{
    ...
    private static readonly Regex s_cefStructRegex = new(
        @"^_cef_\w+_t$", RegexOptions.Compiled);

    // Regex to detect CEF enum names: cef_*_t (but not _cef_*_t)
    private static readonly Regex s_cefEnumRegex = new(
        @"^cef_\w+_t$", RegexOptions.Compiled);
        ...
        out string? cefObjectName)
    {
        ...
        if (isPointer && s_cefStructRegex.IsMatch(baseType))
        {
            isCefObject = true;
        ...
        if (!isPointer && s_cefEnumRegex.IsMatch(baseType))
        {
            return baseType;
        }
    ...
    /// </summary>
    public static bool IsStringValueField(string nativeType, string nativeTypeName)
    {
        return nativeType == "_cef_string_utf16_t" ||
               nativeTypeName.Contains("cef_string_t") && !nativeTypeName.Contains("*");
    }

    /// <summary>
    /// Determine if a type is a CEF enum type (cef_*_t, no pointer).
    /// </summary>
    public static bool IsCefEnum(string nativeType)
    {
        return s_cefEnumRegex.IsMatch(nativeType);
    }
