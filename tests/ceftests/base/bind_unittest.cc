...
#include "tests/gmock/include/gmock/gmock.h"
...
TEST_F(BindTest, UnwrapUnretained) {
  ...
  EXPECT_EQ(&i, cef_internal::Unwrap(unretained));
  EXPECT_EQ(&i, cef_internal::Unwrap(std::move(unretained)));
}

TEST_F(BindTest, UnwrapRetainedRef) {
  auto p = MakeRefCounted<RefCountedData<int>>();
  auto retained_ref = RetainedRef(p);
  EXPECT_EQ(p.get(), cef_internal::Unwrap(retained_ref));
  EXPECT_EQ(p.get(), cef_internal::Unwrap(std::move(retained_ref)));
}
...
TEST_F(BindTest, UnwrapOwned) {
  {
    int* p = new int;
    auto owned = Owned(p);
    EXPECT_EQ(p, cef_internal::Unwrap(owned));
    EXPECT_EQ(p, cef_internal::Unwrap(std::move(owned)));
  }

  {
    ...
    EXPECT_EQ(raw_p, cef_internal::Unwrap(owned));
    EXPECT_EQ(raw_p, cef_internal::Unwrap(std::move(owned)));
  }
}

TEST_F(BindTest, UnwrapPassed) {
  int* p = new int;
  auto passed = Passed(std::unique_ptr<int>(p));
  EXPECT_EQ(p, cef_internal::Unwrap(passed).get());

  p = new int;
  EXPECT_EQ(p, cef_internal::Unwrap(Passed(std::unique_ptr<int>(p))).get());
}
...
// =============================================================================
// SKIPPED TESTS from Chromium's base/functional/bind_unittest.cc
// =============================================================================
// BindTest.RawPtrReceiver - Uses raw_ptr<>
// BindTest.UnretainedRawRefReceiver - Uses raw_ref<>
// BindDeathTest.NullCallback - Death test (uses EXPECT_CHECK_DEATH)
// BindDeathTest.NullFunctionPointer - Death test (uses EXPECT_DCHECK_DEATH)
// BindDeathTest.NullCallbackWithoutBoundArgs - Death test
// BindDeathTest.BanFirstOwnerOfRefCountedType - Death test, uses raw_ptr
// BindUnretainedDanglingTest.* - All tests use raw_ptr dangling detection
// BindUnretainedDanglingDeathTest.* - All tests use raw_ptr and death tests
